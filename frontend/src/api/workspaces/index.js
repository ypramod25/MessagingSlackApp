...
export const deleteWorkspaceRequest = async ({token, workspaceId}) => {
    ...
    try{
        ...
        console.log('Response in deleting workspace request is : ', response);
        return response?.data?.data;
    } catch (error) {
...
export const updateWorkspaceRequest = async ({workspaceId, token, name}) => {
    try{
        ...
        console.log('Response in updating workspace request is : ', response);
        return response?.data?.data;
    } catch (error) {
...
export const addChannelToWorkspaceRequest = async ({workspaceId, token, name}) => {
    try{
        const response = await axios.put(`/workspaces/${workspaceId}/channels`, {name}, {
            headers: {
                'x-access-token':token
            }
        });
        console.log('Response in adding channel to workspace request is : ', response);
        return response?.data?.data;
    } catch (error) {
...
export const resetJoinCodeRequest = async ({workspaceId, token}) => {
    try{
        ...
        console.log('Response in reseting join code of workspace request is : ', response);
        return response?.data?.data;
    } catch (error) {
        console.log('Error in reseting join code to workspace request', error);
        throw error.response.data;
    }
}
...
export const joinWorkspaceRequest = async ({workspaceId, joinCode, token}) => {
    try {
        ...
        console.log('Error in joining the workspace', error);
        throw error.response.data;
    }
}